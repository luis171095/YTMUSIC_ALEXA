...
def get_welcome_response(event):
    list_created = False # create_list(event, 'YouTube')
    list_created = False #create_list(event, 'YouTube Favorites', ['Add shortcuts to your favorite videos or playlists like this:', 'that song I like | https://youtu.be/gJLIiF15wjQ', 'super awesome playlist | https://www.youtube.com/playlist?list=PL1EQjK4xc6hsirkCQq-MHfmUqGMkSgUTn'])
...
# Palabras que delatan una compilacion / mix largo en vez de una cancion individual.
# Se usan con limites de palabra (\b) para no castigar "remix", "mixed", etc.
_COMPILATION_RE = re.compile(
    r'\b('
    r'mix|megamix|mega mix|compilation|compilad\w*|compilaci\w*|'
    r'full album|album completo|[aá]lbum completo|disco completo|'
    r'best of|lo mejor de|grandes [eé]xitos|sus mejores|greatest hits|'
    r'\d+\s*h(?:ou)?rs?|\d+\s*horas?|nonstop|non[- ]stop|'
    r'all songs|todas las canciones|playlist|continuous|enganchados|'
    r'mejores canciones'
    r')\b',
    re.IGNORECASE)
...
def _song_rank(item):
    """Menor puntaje = mejor candidato a cancion individual. Estable: empata por
    el orden de relevancia original de YouTube."""
    snippet = item.get('snippet', {}) or {}
    title = snippet.get('title') or ''
    channel = (snippet.get('channelTitle') or '').lower()
    score = 0
    if _COMPILATION_RE.search(title):
        score += 10            # parece compilacion/mix -> al fondo
    if channel.endswith('- topic'):
        score -= 3             # canal "Artista - Topic": audio individual oficial
    tl = title.lower()
    if any(k in tl for k in ('official audio', 'audio oficial', 'video oficial',
...
def video_search(query=None, relatedToVideoId=None, channelId=None):
    ...
    # Solo re-rankeamos busquedas de cancion por texto (no "mas como esta" ni canal),
    # para que la primera reproducida sea una cancion y no una compilacion de 2h.
    if query and not relatedToVideoId:
        items = sorted(items, key=_song_rank)  # sort estable: conserva relevancia en empates
    videos = []
...
def get_url_and_title_youtube_dl(id, retry=True):
    ...
    # Propiedades base de yt-dlp (proxy + cookies opcionales)
    base_properties = {
        'quiet': True,
        'no_warnings': False,
        'cachedir': '/tmp/',
    ...
    if 'proxy_enabled' in environ and 'proxy' in environ and environ['proxy_enabled'].lower() == 'true':
        base_properties['proxy'] = environ['proxy']

    # (1) Cookies: desde la variable de entorno 'cookiefile' o desde un cookies.txt
    # empaquetado junto al lambda_function.py. Si no existe, simplemente no se usan.
    cookiefile = environ.get('cookiefile')
    if not cookiefile:
        default_cookies = os.path.join(os.path.dirname(__file__), 'cookies.txt')
        if os.path.exists(default_cookies):
            cookiefile = default_cookies
    if cookiefile and os.path.exists(cookiefile):
        # yt-dlp reescribe el cookiefile al cerrar; /var/task es de solo lectura,
        # asi que copiamos las cookies a /tmp (escribible) y usamos esa copia.
        try:
            import shutil
            tmp_cookies = '/tmp/cookies.txt'
            if not os.path.exists(tmp_cookies):
                shutil.copy(cookiefile, tmp_cookies)
            cookiefile = tmp_cookies
        except Exception as e:
            logger.info('No se pudo copiar cookies a /tmp: ' + e.__str__())
        logger.info('Using cookies file: ' + cookiefile)
        base_properties['cookiefile'] = cookiefile

    # (2) Clientes alternativos. 'android_vr' (Oculus Quest, clientId 28) PRIMERO:
    # devuelve URLs reproducibles COMPLETAS sin candado PO token y sin descifrar firma
    # (truco de la familia InnerTube/OpenTune). Los demas clientes (ios/android) entregan
    # URL pero el stream se corta a ~1MB por el muro PO-token/SABR de YouTube; android_vr
    # NO tiene ese limite -> reproduce la cancion completa. 'missing_pot' incluye formatos
    # que YouTube oculta sin PoToken.
    clients = environ.get('player_clients', 'android_vr,ios,android,tv,mweb,web').split(',')
    clients = [c.strip() for c in clients if c.strip()]

    # Frases que indican que el VIDEO en si no esta disponible (no un fallo de formato).
    unavailable_markers = [
        'video unavailable', 'private video', 'this video is unavailable',
        'video is not available', 'has been removed', 'account associated',
        'members-only', 'who has blocked it', 'requires payment',
    ]

    def pick_url(info):
        # Devuelve (url, title) REPRODUCIBLE o (None, None). Solo formatos con 'url'.
        if not info:
            return None, None
        ...
        if video_or_audio[1] == 'audio':
            # Preferencia para Alexa: mejor m4a/AAC-LC (itag 140 128k > 139 48k) por abr,
            # luego audio mp4a > HLS > progresivo mp4 con audio.
            m4as = [f for f in formats if f.get('vcodec') == 'none' and f.get('ext') == 'm4a']
            if m4as:
                best = max(m4as, key=lambda f: f.get('abr') or f.get('tbr') or 0)
                logger.info('audio m4a: ' + str(best.get('format_id')))
                return best['url'], title
            for f in formats:
                if f.get('vcodec') == 'none' and (f.get('acodec') or '').startswith('mp4a'):
                    logger.info('audio mp4a: ' + str(f.get('format_id')))
                    return f['url'], title
            for f in formats:
                if 'm3u8' in (f.get('protocol') or ''):
                    logger.info('audio hls: ' + str(f.get('format_id')))
                    return f['url'], title
            for f in formats:
                if f.get('acodec') not in (None, 'none') and f.get('ext') == 'mp4':
                    logger.info('audio progresivo mp4: ' + str(f.get('format_id')))
                    return f['url'], title
            for f in formats:
                # Solo audio REAL: vcodec none Y acodec presente (excluye storyboards/imagenes).
                if f.get('vcodec') == 'none' and f.get('acodec') not in (None, 'none'):
                    logger.info('audio fallback: %s ext=%s' % (f.get('format_id'), f.get('ext')))
                    return f['url'], title
        else:
            for f in formats:
    ...
    for client in clients:
        ...
        properties['extractor_args'] = {'youtube': {
            'player_client': [client],
            'formats': ['missing_pot'],
        }}
        properties['ignore_no_formats_error'] = True
        try:
            logger.info('Trying yt-dlp with player_client=' + client)
            with youtube_dl.YoutubeDL(properties) as ydl:
                info = ydl.extract_info(yt_url, download=False)
        except Exception as e:
            msg = e.__str__()
            low = msg.lower()
            # Solo lo tratamos como "video no disponible" si NO es un error de formato.
            if 'format' not in low and any(m in low for m in unavailable_markers):
                logger.info(id+' is unavailable')
                return None, None
            logger.info('yt-dlp error (player_client=%s): %s' % (client, msg))
            continue
        # Solo nos sirve si ademas de extraer, hay una URL REPRODUCIBLE.
        url, title = pick_url(info)
        if url:
            logger.info('yt-dlp OK con player_client=%s -> %s' % (client, str(title)))
            return url, title
        n = len((info or {}).get('formats') or [])
        logger.info('player_client=%s extrajo pero SIN url reproducible (%d formatos)' % (client, n))

    # Ningun cliente dio URL reproducible -> mirror o pytube
